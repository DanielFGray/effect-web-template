	...
	(handlers) =>
		handlers
			...
			.handle('getForInvitation', ({ path: { invitationId }, urlParams }) =>
				Organizations.forInvitation(invitationId, urlParams.code).pipe(withAuthContext),
			)
			.handle('acceptInvitation', ({ path: { invitationId }, payload }) =>
				Organizations.acceptInvitation(invitationId, payload.code).pipe(withAuthContext),
			)
			.handle('transferOwnership', ({ path: { orgId }, payload }) =>
				Organizations.transferOwnership(orgId, payload.userId).pipe(withAuthContext),
			)
			.handle('transferBillingContact', ({ path: { orgId }, payload }) =>
				Organizations.transferBillingContact(orgId, payload.userId).pipe(withAuthContext),
			),
)
