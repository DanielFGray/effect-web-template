...
suite('posts HTTP flow', () => {
	...
	it.live('POST /posts creates a post for authenticated user', () =>
		Effect.gen(function* () {
			...
			const created =
				yield* HttpClientResponse.schemaBodyJson(CreatePostResponse)(createResponse)
	...
	it.live('GET /posts/:id returns the created post', () =>
		Effect.gen(function* () {
			...
			const post = yield* HttpClientResponse.schemaBodyJson(S.NullOr(PostWithDetails))(
				getResponse,
			)
	...
	it.live('GET /posts list includes the created post', () =>
		Effect.gen(function* () {
			...
			const listResponse = yield* clientWithCookies.get(`${yield* baseUrl}/posts`)
			expect(listResponse.status).toBe(200)

			const posts = yield* HttpClientResponse.schemaBodyJson(S.Array(PostWithDetails))(
				listResponse,
			)
	...
	it.live('POST /posts fails without session cookie', () =>
		Effect.gen(function* () {
			...
			const result = yield* HttpClientRequest.post(`${yield* baseUrl}/posts`).pipe(
				HttpClientRequest.bodyJson({
