...
suite("organizations HTTP flow", () => {
  it.live("two-user org lifecycle: create, invite, accept, transfer, delete", () =>
    Effect.gen(function* () {
      ...
      const userA = yield* HttpClientResponse.schemaBodyJson(
        S.Struct({ id: S.UUID }),
      )(registerA);
      ...
      const patched = yield* HttpClientResponse.schemaBodyJson(
        Organization.select,
      )(patchResponse);
      ...
      const userB = yield* HttpClientResponse.schemaBodyJson(
        S.Struct({ id: S.UUID }),
      )(registerB);
      ...
      const acceptResponse = yield* HttpClientRequest.post(
        ...
        HttpClientRequest.bodyJson({ code: null }),
        Effect.flatMap(clientB.execute),
      );
      ...
      const afterDelete = yield* clientB.get(
        `${yield* baseUrl}/organizations/${orgId}`,
      );
