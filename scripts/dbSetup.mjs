...
;(async function main() {
	...
	try {
		...
		if (process.env.NODE_ENV === 'production') {
			await sql`create role ${sql.unsafe(
				DATABASE_OWNER,
			)} with login password '${sql.unsafe(DATABASE_OWNER_PASSWORD)}' noinherit`
			console.log(`CREATE ROLE ${DATABASE_OWNER}`)
		} else {
			await sql`create role ${sql.unsafe(
				DATABASE_OWNER,
			)} with login password '${sql.unsafe(DATABASE_OWNER_PASSWORD)}' superuser`
			console.log(`CREATE ROLE ${DATABASE_OWNER} SUPERUSER`)
		}

		await sql`grant all privileges on database ${sql.unsafe(
			DATABASE_NAME,
		)} to ${sql.unsafe(DATABASE_OWNER)}`
		console.log(`GRANT ${DATABASE_OWNER}`)

		await sql`create role ${sql.unsafe(
			DATABASE_AUTHENTICATOR,
		)} with login password '${sql.unsafe(DATABASE_AUTHENTICATOR_PASSWORD)}' noinherit`
		...
		await sql`grant ${sql.unsafe(DATABASE_VISITOR)} to ${sql.unsafe(
			DATABASE_AUTHENTICATOR,
		)}`
